...
const int numOfBinaries = 7; // Non-commutative operators count twice
    ...
    const int numOfSamples)
{
    ...
    for (int i = 0; i < numOfSamples; ++i) {
        ...
        CS[i + 4 * numOfSamples] = lRes - rRes; CS[i + 5 * numOfSamples] = rRes - lRes;
        CS[i + 6 * numOfSamples] = lRes * rRes;
    }
