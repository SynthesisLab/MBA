...
tuple<uint32_t *, uint32_t *, int> readJsonFile(const std::string &filename) {
  ...
  for (const auto &example : j["sampling"]) {
    outputData[idx / numVar] = std::stoul(
        example["outputs"]["0"]["value"].get<std::string>(), nullptr, 16);
    for (const auto &value : example["inputs"]) {
      inputData[idx++] =
          std::stoul(value["value"].get<std::string>(), nullptr, 16);
    }
  }
...
__device__ void makeUnqChk(uint32_t *CS, uint64_t &seed,
...
__global__ void hashSetInit(const int numOfSamples, hash_set_t hashSet) {
  ...
  const auto group =
      warpcore::cg::tiled_partition<1>(warpcore::cg::this_thread_block());
...
template <Op op>
__device__ void evaluateFormula(uint32_t *CS, char *d_MBACache, int ldx,
                                int rdx, const int numOfSamples) {
...
template <class hash_set_t>
__device__ bool processUniqueCS(uint32_t *CS, const int numOfSamples,
                                hash_set_t &hashSet) {
...
template <Op op>
__device__ void insertInCache(uint32_t *CS, bool CSUniq, int tid, int ldx,
                              int rdx, int llen, int rlen, int len,
                              const int numOfSamples, char *d_MBACache,
                              char *d_temp_MBACache, char *d_MBAFormula) {

  if (CSUniq) {

    for (int i = 0; i < llen - 1; ++i)
      d_temp_MBACache[tid * len + i] = d_MBACache[ldx + i];
    for (int i = 0; i < rlen - 1; ++i)
      d_temp_MBACache[tid * len + i + llen - 1] = d_MBACache[rdx + i];
    d_temp_MBACache[tid * len + len - 2] = d_opChar[static_cast<int>(op)];
    d_temp_MBACache[tid * len + len - 1] = '\0';

    bool found = true;
    for (int i = 0; found && i < numOfSamples; ++i)
      found = (CS[i] == d_outputData[i]);
    if (found) {
      for (int i = 0; i < len; ++i)
        d_MBAFormula[i] = d_temp_MBACache[tid * len + i];
    }

  } else {

    for (int i = 0; i < len; ++i)
      d_temp_MBACache[tid * len + i] = '?';
  }
}

template <Op op, class hash_set_t>
__global__ void processOperator(char *d_MBACache, char *d_temp_MBACache,
                                const int ldx1, const int ldx2, const int llen,
                                const int rdx1, const int rdx2, const int rlen,
                                const int numOfSamples, char *d_MBAFormula,
                                hash_set_t hashSet) {
  ...
  constexpr bool notCommut =
      (op == Op::Minus || op == Op::Div || op == Op::Mod || op == Op::LShift ||
       op == Op::RShift);
  ...
  if (tid < maxTid) {
    ...
    insertInCache<op>(CS, CSUniq, modTid, ldx, rdx, llen, rlen, len,
                      numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula);
    ...
    if (notCommut) {
      ...
      insertInCache<op>(CS, CSUniq, modTid, rdx, ldx, rlen, llen, len,
                        numOfSamples, d_MBACache, d_temp_MBACache,
                        d_MBAFormula);
    }
  }
}

bool storeUniqueMBAs(int N, uint64_t &lastIdx, int MBALen,
                     const int numOfSamples, const int MBACacheCapacity,
  ...
  new_end_ptr =
      thrust::remove(d_temp_MBACache_ptr, d_temp_MBACache_ptr + space, '?');
...
template <Op op, class hash_set_t>
bool generateMBAs(int MBALen, int *startPoints, const int MBACacheCapacity,
                  const int temp_MBACacheCapacity, uint64_t &allMBAs,
                  uint64_t &lastIdx, const int numOfSamples, char *d_MBACache,
                  char *d_temp_MBACache, char *d_MBAFormula, char *MBAFormula,
                  const char *opStr, hash_set_t hashSet) {
  ...
  if (isUnary) {
...
#endif
        int Blc = (N + 1023) / 1024;
        processOperator<op><<<Blc, 1024>>>(d_MBACache, d_temp_MBACache, x, y,
                                           MBALen, 0, 0, 1, numOfSamples,
                                           d_MBAFormula, hashSet);
        checkCuda(cudaPeekAtLastError());
        checkCuda(cudaMemcpy(MBAFormula, d_MBAFormula, 64 * sizeof(char),
                             cudaMemcpyDeviceToHost));
        if (MBAFormula[0] != '\0')
          return true;
        uint64_t oldIdx = lastIdx;
        if (storeUniqueMBAs(N, lastIdx, MBALen, numOfSamples, MBACacheCapacity,
                            d_MBACache, d_temp_MBACache))
          return true;
    ...
    for (int i = 1; 2 * i <= MBALen - 1; ++i) {
      ...
      int N =
          ((idx4 - idx3 + 1) / (MBALen - i)) * ((idx2 - idx1 + 1) / (i + 1));
      int lim = temp_MBACacheCapacity /
                (((idx2 - idx1 + 1) / (i + 1)) * (MBALen + 1));
      if (notCommut)
        lim /= 2;
...
#ifndef MEASUREMENT_MODE
          printf(
              "Length %-2d | %s | StoredMBAs: %-11lu | ToBeChecked: %-10d \n",
              MBALen, opStr, allMBAs, modN);
#endif
          int Blc = (N + 1023) / 1024;
          processOperator<op><<<Blc, 1024>>>(
              d_MBACache, d_temp_MBACache, idx1, idx2, i + 1, x, y, MBALen - i,
              numOfSamples, d_MBAFormula, hashSet);
          checkCuda(cudaPeekAtLastError());
          checkCuda(cudaMemcpy(MBAFormula, d_MBAFormula, 64 * sizeof(char),
                               cudaMemcpyDeviceToHost));
          if (MBAFormula[0] != '\0')
            return true;
          uint64_t oldIdx = lastIdx;
          if (storeUniqueMBAs(modN, lastIdx, MBALen, numOfSamples,
                              MBACacheCapacity, d_MBACache, d_temp_MBACache))
            return true;
          allMBAs += (lastIdx - oldIdx) / (MBALen + 1);
          x = y + 1;
        } while (y < idx4);
...
void printCharsFromGPU(char *d_array, int end_index) {
  ...
  cudaMemcpy(h_copy, d_array, (end_index + 1) * sizeof(char),
             cudaMemcpyDeviceToHost);
  cout << "Output: " << endl;
...
string MBA(const int maxLen, const int numOfSamples, uint32_t *inputData,
           uint32_t *outputData) {
  ...
  checkCuda(cudaMemcpyToSymbol(d_inputData, inputData,
                               numVar * numOfSamples * sizeof(uint32_t)));
  checkCuda(cudaMemcpyToSymbol(d_outputData, outputData,
                               numOfSamples * sizeof(uint32_t)));
  ...
  for (int i = 0; i < numVar; ++i) {
    ...
    for (int j = 0; j < numOfSamples; j++) {
      if (!(inputData[j * numVar + i] == outputData[j]))
        found = false;
    }
    allMBAs++;
    if (found)
  ...
  checkCuda(cudaMemcpy(d_MBACache, MBACache, 2 * numVar * sizeof(char),
                       cudaMemcpyHostToDevice));

  using hash_set_t =
      warpcore::HashSet<uint64_t,        // Key type
                        uint64_t(0) - 1, // Empty key
                        uint64_t(0) - 2, // Tombstone key
                        warpcore::probing_schemes::QuadraticProbing<
  ...
  for (int MBALen = 2; MBALen <= maxLen; ++MBALen) {
    ...
    lastRound = generateMBAs<Op::Not>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(~)  ", hashSet);
    if (lastRound)
      break;

    lastRound = generateMBAs<Op::And>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(&)  ", hashSet);
    if (lastRound)
    ...
    lastRound = generateMBAs<Op::LShift>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(<<) ", hashSet);
    if (lastRound)
    ...
    lastRound = generateMBAs<Op::RShift>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(>>) ", hashSet);
    if (lastRound)
    ...
    lastRound = generateMBAs<Op::Neg>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(Neg)", hashSet);
    if (lastRound)
      break;

    lastRound = generateMBAs<Op::Plus>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(+)  ", hashSet);
    if (lastRound)
    ...
    lastRound = generateMBAs<Op::Minus>(
        MBALen, startPoints, MBACacheCapacity, temp_MBACacheCapacity, allMBAs,
        lastIdx, numOfSamples, d_MBACache, d_temp_MBACache, d_MBAFormula,
        MBAFormula, "(-)  ", hashSet);
    if (lastRound)
