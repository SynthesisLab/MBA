    ...
    const int numOfSamples)
{
    ...
    for (int i = 0; i < numOfSamples; ++i) {
        uint64_t val = simpleHash(CS[i + idx]) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
    ...
    int* d_FinalMBAIdx)
{
    ...
    const int shift = modTid * numOfSamples;
    int idx{}; bool found;
    ...
    for (int j = 0; j < numOfFormulas; ++j) {
        ...
        if (d_CSUnq[j]) {
            ...
            found = true;
            for (int i = 0; found && i < numOfSamples; ++i) found = (CS[i + idx] == d_outputData[i]);
            if (found) atomicCAS(d_FinalMBAIdx, -1, modTid);

        } else {

            for (int i = 0; i < numOfSamples; ++i) {
                d_temp_boolCache[shift + i + idx] = false;
            }
    ...
    hash_set_t hashSet)
{
    ...
    if (tid < maxTid) {
        ...
        const int numOfFormulas = numOfBinaries;
        uint32_t CS[maxNumOfSamples * numOfFormulas];
        bool d_CSUnq[numOfUnaries];

        applyBinOp(CS, d_MBACache, ldx, rdx, numOfSamples);
        processUniqueCS(CS, d_CSUnq, numOfSamples, numOfFormulas, hashSet);
        insertInCache(
            CS, d_CSUnq, tid, ldx, rdx, numOfSamples, numOfFormulas, d_temp_MBACache, d_temp_boolCache,
