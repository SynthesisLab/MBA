...
enum class Op { Not, And, Or, Xor, Neg, Plus, Minus, Mul };
    ...
    hash_set_t hashSet)
{
    ...
    constexpr bool notCommut = (op == Op::Minus);
    ...
    const char* opStr, int opIdx)
{
    ...
    if (isUnary) {
        ...
        int idx1 = startPoints[(MBALen - 1) * 8];
        int idx2 = startPoints[MBALen * 8] - 1;
        int N = idx2 - idx1 + 1;
        ...
        if (N) {
            ...
            do {
                ...
                allMBAs += N;
                if (*FinalMBAIdx != -1) { startPoints[MBALen * 8 + opIdx] = INT_MAX; return true; }
        ...
        startPoints[MBALen * 8 + opIdx] = lastIdx;

    } else {

        for (int i = 1; 2 * i <= MBALen - 1; ++i) {

            int idx1 = startPoints[i * 8];
            int idx2 = startPoints[(i + 1) * 8] - 1;
            int idx3 = startPoints[(MBALen - i - 1) * 8];
            int idx4 = startPoints[(MBALen - i) * 8] - 1;
    ...
    uint64_t* outputData)
{
    ...
    // Unary operators : ~, Neg
    // Binary operators : &, |, ^, +, -, *
    int* startPoints = new int[(maxLen + 2) * 8]();
    startPoints[(2 * 8) - 1] = lastIdx;
    startPoints[2 * 8] = lastIdx;
    ...
    for (int MBALen = 2; MBALen <= maxLen; ++MBALen) {
        ...
        lastRound = generateMBAs<Op::Neg>(MBALen, startPoints, temp_MBACacheCapacity, MBACacheCapacity, allMBAs, lastIdx,
            numOfSamples, d_MBACache, d_temp_MBACache, d_temp_leftIdx, d_temp_rightIdx,
            d_leftIdx, d_rightIdx, d_FinalMBAIdx, FinalMBAIdx, hashSet, "(Neg)", 5);
        if (lastRound) break;
        ...
        lastRound = generateMBAs<Op::Minus>(MBALen, startPoints, temp_MBACacheCapacity, MBACacheCapacity, allMBAs, lastIdx,
            numOfSamples, d_MBACache, d_temp_MBACache, d_temp_leftIdx, d_temp_rightIdx,
            d_leftIdx, d_rightIdx, d_FinalMBAIdx, FinalMBAIdx, hashSet, "(-)  ", 7);
        if (lastRound) break;
...
int main(int argc, char* argv[]) {
    ...
    if (atoi(argv[2]) < 1 || atoi(argv[2]) > 100) {
        printf("Argument maxLen = %s should be between 1 and 100\n", argv[2]);
        return 0;
    }
